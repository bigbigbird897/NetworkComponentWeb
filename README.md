...
| 2026-09-23 | Socket 客户端页升级
| 2026-09-24 | MQTT 页升级
| 2026-09-24（二次调整）|
| 2026-09-24（三次调整）| MQTT 主题历史下拉：发布主题/订阅主题/发送主题/应答主题四个输入框均接入 `<datalist>`，操作成功后主题自动写入 localStorage（nc_mqtt_pub/sub/send/reply，各保留最近 20 条、去重、最新在前），下次点输入框即可下拉选择历史主题，仍可手动输入新主题。 | MQTT 页交互优化：①「发布」与「托管订阅」拆为两张独立卡片（发布主题/消息 与 订阅主题 分离，订阅走独立 subTopic 字段）；②修复“发送并等待”结果被轮询启动提示覆盖看不到的问题——结果改为固定显示在「发送并等待应答」卡片内（isSuccess/isTimeout/应答内容常驻）；③轮询启动不再写共享结果区，新增 beforeDestroy 销毁前停止轮询定时器。 |：① 新增「订阅主题消息」卡片——显示当前订阅主题（mq.topic）收到的消息，订阅成功后自动开始 2 秒轮询后端 `GetReceivedMessages`，取消订阅自动停止并清空；② 新增「应答主题消息」卡片——显示其他客户端发往应答主题（mq.topicReply）的消息，点击“发送并等待”后自动开始轮询；③ 两张卡片均带 开始轮询/停止轮询/刷新/清空 按钮与轮询状态指示。`api/console.js` 同步新增 `mqtt.received`。配套后端：MqttClientService 新增消息缓冲与 `GetReceivedMessagesAsync`，`PublishAndWaitReplyAsync` 自动订阅答复主题（修复收不到应答），应答支持纯文本兼容返回。 |：① 新增「仅发送字符串(不等待)」与「仅发送HEX(不等待)」按钮（对应后端 `SendOnlyString` / `SendOnly`）；② 新增长连接管理卡片（打开/关闭/刷新状态，对应后端 `OpenLongConnection / CloseLongConnection / GetLongConnectionStatus`）；③ HEX 输入说明改为“Data 支持数组或 HEX 字符串”。`api/console.js` 同步新增 `sendOnlyString / openLong / closeLong / longStatus`。 |
